...
?>
  ...
  <head>
    ...
    <title>Laporan Bantuan Sosial BLT <?= "({$dari_tanggal} s.d. {$sampai_tanggal})" ?></title>
  </head>
  ...
  <body class="bg-white">
    ...
    $stmt_bantuan_sosial_blt = mysqli_stmt_init($connection);
    $query_bantuan_sosial_blt = 
      "SELECT
      ...
      WHERE a.tipe_bantuan='BLT' AND a.created_at BETWEEN ? AND ?
      ORDER BY a.id DESC";
      
    mysqli_stmt_prepare($stmt_bantuan_sosial_blt, $query_bantuan_sosial_blt);
    mysqli_stmt_bind_param($stmt_bantuan_sosial_blt, 'ss', $dari_tanggal, $sampai_tanggal);
    mysqli_stmt_execute($stmt_bantuan_sosial_blt);

    $result = mysqli_stmt_get_result($stmt_bantuan_sosial_blt);
    $bantuan_sosial_blts = mysqli_fetch_all($result, MYSQLI_ASSOC);

    mysqli_stmt_close($stmt_bantuan_sosial_blt);
    mysqli_close($connection);
    ?>

    <h4 class="text-center mb-4">Laporan Bantuan Sosial BLT <?= "({$dari_tanggal} s.d. {$sampai_tanggal})" ?></h4>
    ...
    <table class="table table-striped table-bordered table-sm">
      ...
      <tbody>
        ...
        <?php else: ?>
          ...
          <?php foreach($bantuan_sosial_blts as $bantuan_sosial_blt) : ?>

            <tr>
              <td><?= $no++ ?></td>
              <td><?= $bantuan_sosial_blt['nik'] ?></td>
              <td><?= $bantuan_sosial_blt['nama_lengkap'] ?></td>
              <td><?= $bantuan_sosial_blt['tipe_bantuan'] ?></td>
              <td>
                <?=
                $bantuan_sosial_blt['status_pengajuan'] === 'sudah_diproses'
                  ? '<span class="badge bg-green-soft text-green">Sudah Diproses</span>'
                  : '<span class="badge bg-red-soft text-red">Belum Diproses</span>';
                ?>
              </td>
              <td><?= $bantuan_sosial_blt['keterangan_pengajuan'] ?></td>
              <td><?= $bantuan_sosial_blt['tgl_pengajuan'] ?></td>
            </tr>
